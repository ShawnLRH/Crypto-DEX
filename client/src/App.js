...
function App({web3, accounts, contracts}) {
  ...
  const listenToTrades = token => {
    ...
    const listener = contracts.dex.events.NewTrade(
      ...
      .on('data', newTrade => {
        ...
        tradeIds.add(newTrade.returnValues.tradeIds)
  ...
  const deposit = async amount => {
    ...
    await contracts.dex.methods.deposit(
      amount, web3.utils.fromAscii(user.selectedToken.ticker)
    )
    .send({from: user.accounts[0]})
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [balances, orders] = await Promise.all([
        await getBalances(accounts[0], tokens[0]),
        getOrders(tokens[0])
      ]);
  ...
  useEffect(() => {
    const init = async () => {
      const [balances, orders] = await Promise.all([
        await getBalances(accounts[0], user.selectedToken),
